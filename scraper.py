    ...
    if os.path.exists(CHECKPOINT_FILE):
        try:
            with open(CHECKPOINT_FILE, 'r', encoding='utf-8') as f:
                ...
                print(f"Loaded checkpoint: {checkpoint.get('completed_listings', 0)} listings completed")
                return checkpoint
    ...
    if os.path.exists(PROGRESS_FILE):
        try:
            with open(PROGRESS_FILE, 'r', encoding='utf-8') as f:
                ...
                print(f"Loaded {len(listings)} listings from progress file")
                return listings
        except Exception as e:
...
async def get_page(session, url, retries=5):
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            print(f"Attempt {attempt + 1} failed for {url}: {type(e).__name__}")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        if checkpoint.get('phase') == 'collecting' or not checkpoint.get('pending_urls'):
            ...
            if resume_page == start_page:
                ...
                print(f"  Page 1: {len(urls_from_first)} listings")
            ...
            for page_num in range(max(resume_page, start_page + 1), total_pages + 1):
                ...
                if urls:
                    ...
                    print(f"  Page {page_num}/{total_pages}: {len(urls)} listings (total: {len(all_listing_urls)})")
                else:
                    print(f"  Page {page_num}/{total_pages}: 0 listings")
        ...
        for i, url in enumerate(unique_urls):
            ...
            try:
                ...
                if (i + 1) % 10 == 0 or i == total - 1:
                    print(f"  Progress: {i + 1}/{total} ({scraped_count} successful, {len(failed_urls)} failed)")
...
async def main():
    ...
    print(f"\n{'='*50}")
    print(f"YENIEMLAK.AZ SCRAPER")
    print(f"{'='*50}")
    print(f"Started at: {datetime.now()}")
    print(f"Pages: {args.start_page} to {args.end_page or 'all'}")
    print(f"Max concurrent requests: {args.concurrent}")
    print(f"Checkpoint interval: every {CHECKPOINT_INTERVAL} listings")
